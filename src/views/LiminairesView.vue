    ...
    <div class="char-detail-panel lim-editor-panel">
      ...
      <template v-else>
        ...
        <div class="lim-toolbar" v-if="editor">
          ...
          <div class="toolbar-group">
            <button class="toolbar-btn" :class="{ active: editor.isActive({ textAlign: 'left' }) }" @click="editor.chain().focus().setTextAlign('left').run()">≡</button>
            <button class="toolbar-btn" :class="{ active: editor.isActive({ textAlign: 'center' }) }" @click="editor.chain().focus().setTextAlign('center').run()">≡</button>
            <button class="toolbar-btn" :class="{ active: editor.isActive({ textAlign: 'right' }) }" @click="editor.chain().focus().setTextAlign('right').run()">≡</button>
            <button class="toolbar-btn" :class="{ active: editor.isActive({ textAlign: 'justify' }) }" @click="editor.chain().focus().setTextAlign('justify').run()">≡</button>
          </div>
          <div class="toolbar-sep"></div>
          <div class="toolbar-group">
...
.lim-item:hover { background: var(--bg-card); }
.lim-item.active {
...
.save-status.saved { color: var(--success, #6fcf97); }
...
:deep(.lim-editor h3) { font-size: 15px; font-weight: 600; margin: 0 0 10px; color: var(--text-muted); }
